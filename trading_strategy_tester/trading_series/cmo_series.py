...
class CMO(TradingSeries):
    ...
    -----------
    target : str
        The column in the DataFrame on which the CMO is calculated (e.g., 'Close').

    length : int
        The number of periods over which to calculate the CMO.

    name : str
        The name of the CMO series.
    """

    def __init__(self, ticker: str, target: str = 'Close', length: int = 9):
        """
        ...
        self.target = target  # Set the target column for the CMO calculation
        self.length = length  # Set the length (number of periods) for the CMO calculation
        self.name = f'{self._ticker}_CMO_{self.target}_{self.length}'  # Define the name for the CMO series
    ...
    def get_data(self, downloader: DownloadModule, df: pd.DataFrame) -> pd.Series:
        ...
        if self.name not in df.columns:
            ...
            cmo_series = cmo(series=new_df[self.target], length=self.length)
