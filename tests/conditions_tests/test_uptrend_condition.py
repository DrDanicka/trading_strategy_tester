...
class TestUptrendCondition(unittest.TestCase):
    ...
    def test_to_string(self):
        ...
        number_of_days = 4
        test_parameter = random.randint(1, 100)
        expected_signal = f'UptrendForXDaysCondition({number_of_days}, {ticker}_TEST_{test_parameter})'

        # Act
        uptrend_condition = UptrendForXDaysCondition(
            TestingSeries(
                ticker,
                pd.Series([1, 2, 2, 4, 5, 1, 7, 8, 9, 10, 3, 12]),
                test_parameter
        ...
        # Assert
        self.assertEqual(uptrend_condition.to_string(), expected_signal)
