...
class TestLessThanCondition(unittest.TestCase):
    ...
    def test_to_string(self):
        ...
        series1_test_parameter = 6
        series2_test_parameter = 17
        expected_signal = f'LessThanCondition({ticker}_TEST_{series1_test_parameter}, {ticker}_TEST_{series2_test_parameter})'

        # Act
        less_than_condition = LessThanCondition(
            TestingSeries(ticker, pd.Series([0, 3, 4, 5, 6]), series1_test_parameter),
            TestingSeries(ticker, pd.Series([1, 2, 3, 4, 5]), series2_test_parameter)
        )

        # Assert
        self.assertEqual(less_than_condition.to_string(), expected_signal)
