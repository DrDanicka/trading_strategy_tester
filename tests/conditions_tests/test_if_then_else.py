...
class TestIfThenElseCondition(unittest.TestCase):
    ...
    def test_to_string(self):
        ...
        expected_string = f'IfThenElseCondition(TestCondition({ticker}_TEST_{test_parameter1}), TestCondition({ticker}_TEST_{test_parameter2}))'

        if_then_else_condition = IfThenElse(
            TestCondition(
                TestingSeries(
                    ticker, pd.Series([True, False, False, True, False]), test_parameter1
                )
        ...
        result_string = if_then_else_condition.to_string()
