...
class TestGreaterThanCondition(unittest.TestCase):
    ...
    def test_to_string(self):
        ...
        series1_test_parameter = 6
        series2_test_parameter = 13
        expected_signal = f'GreaterThanCondition({ticker}_TEST_{series1_test_parameter}, {ticker}_TEST_{series2_test_parameter})'

        # Act
        greater_than_condition = GreaterThanCondition(
            TestingSeries(ticker, pd.Series([0, 3, 3, 5, 6]), series1_test_parameter),
            TestingSeries(ticker, pd.Series([1, 2, 3, 4, 5]), series2_test_parameter)
        )

        # Assert
        self.assertEqual(greater_than_condition.to_string(), expected_signal)
