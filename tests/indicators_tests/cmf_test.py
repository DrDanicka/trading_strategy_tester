...
from trading_strategy_tester.trading_series.cmf_series import CMF
...
class TestCMF(unittest.TestCase):
    ...
    def test_cmf_length_20_tradingview_data(self):
        ...
        ], name='CMF_20').reset_index(drop=True)
        calculated_cmf = cmf(self.data['High'], self.data['Low'], self.data['Close'], self.data['Volume'],
                             20).tail(20).reset_index(drop=True).round(2)
    ...
    def test_cmf_length_10_tradingview_data(self):
        ...
        ], name='CMF_10').reset_index(drop=True)
        calculated_cmf = cmf(self.data['High'], self.data['Low'], self.data['Close'], self.data['Volume'],
                             10).tail(20).reset_index(drop=True).round(2)
        pd.testing.assert_series_equal(calculated_cmf, trading_view_cmf)
    ...
    def test_cmf_series_length_20_tradingview_data(self):
        ...
        trading_view_cmf = pd.Series([
            0.23, 0.19, 0.17, 0.09, 0.07, 0.14, 0.12, 0.19, 0.25, 0.29,
            0.25, 0.26, 0.27, 0.22, 0.22, 0.21, 0.19, 0.22, 0.23, 0.19
        ], name=f'{ticker}_CMF_20').reset_index(drop=True)
        cmf_series = CMF(ticker, 20)
